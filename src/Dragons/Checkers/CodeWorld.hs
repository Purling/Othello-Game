...
{-|
Module      : Dragons.Checkers.Codeworld
Description : CodeWorld interface for the Checkers game
Copyright   : (c) 2020 The Australian National University
License     : AllRightsReserved
-}
module Dragons.Checkers.CodeWorld where
...
drawScore st
  ...
  where
    p1 = "Black: " <> T.pack (show p1count)
    p2 = "Red: " <> T.pack (show p2count)
...
drawHighlights model st = case model of
  ...
  where
    ...
    highlight (Location x y) = translated (fromIntegral x) (-(fromIntegral y)) $
      colored purple (thickCircle 0.1 0.5)
...
drawLastMove st mode = case mode of
  AwaitingMove _ (Just (_, Move (Location fx fy) (Location tx ty))) _ ->
    centreGrid st $ pictures ((map showCapture $ captured st) ++
      [ translated fx' (-fy') purpleCircle
      , translated tx' (-ty') (colored purple (solidCircle 0.2))
      , colored purple (thickPolyline 0.1 [(fx', -fy'), (tx', -ty')])
      ])
    where
      purpleCircle = pictures
        [ colored (purple) $ thickCircle 0.4 0.05
        , colored (dull green) $ solidCircle 0.25
        ]

      showCapture (Location x y) = translated x' (-y') $ coloured purple $ solidCircle 0.5
...
drawSquare sq = case sq of
  Empty -> square
  Blank -> colored (RGB 245 245 220) (solidRectangle 1 1)
  Piece p Pawn -> case p of
    Player1 -> coloured black (solidCircle 0.5) & square 
    Player2 -> coloured red (solidCircle 0.5) & square 
  Piece p King -> crown & (drawSquare (Piece p Pawn)) & square
  where 
    square = coloured (dull green) (solidRectangle 1 1)
    crown = coloured (RGB 255 215 0) (solidCircle 0.33) 
                 
-- | Labels for pieces that match how we draw them.
pieceName :: Player -> Text
pieceName = player "Black" "Red"
